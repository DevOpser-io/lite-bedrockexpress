...
async function initializeGmailTransporter() {
  try {
    ...
    const mailServer = await getSecret(process.env.MAIL_SERVER);
    const mailPort = parseInt(await getSecret(process.env.MAIL_PORT));
    const mailUsername = await getSecret(process.env.MAIL_USERNAME);
    const mailPassword = await getSecret(process.env.MAIL_PASSWORD_SECRET_NAME);
    const mailDefaultSender = await getSecret(process.env.MAIL_DEFAULT_SENDER).catch(() => mailUsername);

    console.log(`Gmail config retrieved - Server: ${mailServer}, Port: ${mailPort}, User: ${mailUsername}`);
