...
// Supports: *.devopser.io subdomains, custom domains via CUSTOM_DOMAIN env var, and localhost
const corsOptions = {
  origin: function (origin, callback) {
    ...
    // Check *.devopser.io subdomains (both org-specific ALB and custom subdomains)
    // Matches: bedrockexpressdemo.devopser.io, org-12345-flask-dev.devopser.io, etc.
    if (/^https:\/\/[a-zA-Z0-9-]+\.devopser\.io$/.test(origin)) {
      return callback(null, true);
    }
...
app.use((req, res, next) => {
  if (req.path === '/mobile-app.html') {
    ...
    // Allow ONLY on org-specific ALB DNS (format: org-XXXXX-flask-dev.devopser.io)
    // This public ALB DNS is used for testing/webhooks/development
    // Block on production domains (custom domains like myapp.devopser.io or myapp.com)
    const isOrgALB = /^org-[a-zA-Z0-9]+-flask-dev\.devopser\.io$/.test(hostname);
